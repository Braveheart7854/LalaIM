        ...
        foreach ($sockets as $socket) {
            ...
            if ($socket == $this->master) {
                ...
                if ($recv_msg['type'] == 'login'){
                    $this->users[$recv_msg['content']] = (int)$socket;

                    $this->broadcast($msg);
                }elseif($recv_msg['type'] == 'logout'){
                    unset($this->users[$recv_msg['content']]);
                    
                    $this->broadcast($msg);
                }else{
                    if (isset($this->users[$recv_msg['accept']])){
                        $accept_socket = $this->sockets[$this->users[$recv_msg['accept']]];
                        socket_write($accept_socket['resource'],$msg,strlen($msg));
                        socket_write($socket,$msg,strlen($msg));
                    }else{
                        $recv_msg['content'] = "对方目前不在线\n";
                        $info = self::dealMsg($socket, $recv_msg);
...
//$ws = new WebSocket("192.168.67.183", "6001");
$ws = new WebSocket("127.0.0.1", "6001");